...
def plot_decision_boundary(model: torch.nn.Module, X: torch.Tensor, y: torch.Tensor):
    ...
    plt.contourf(xx, yy, y_pred, cmap=plt.get_cmap('RdYlBu'), alpha=0.7)
...
def split_dataset(image_path, train_split=0.8):
    ...
    print(f"Creating train/test split ({train_split:.0%} train, {1-train_split:.0%} test)...")
    ...
    class_dirs = [d for d in image_path.iterdir() if d.is_dir() and d.name in ["C0", "C1", "C2", "C3"]]
